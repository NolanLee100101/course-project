...
class ProfBST
{
    ...
    private:
		...
		Professor* addNewNodeHelper(Professor*,string,string);
		ProfBST(string);
		Professor* createNode(string, string);
		bool searchKey(string);
		Professor * searchKeyHelper(Professor*, string);
		void deleteNode (Professor* root);
};
